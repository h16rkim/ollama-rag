...
import { Controller, Post, Body, Res, Logger, BadRequestException, Inject } from '@nestjs/common';
...
import { BaseStreamHandlerService } from '../common/services/stream/base-stream-handler.service';
import { PROVIDER_TOKENS } from '../common/constants/provider-tokens.constants';
...
export class OllamaController {
  ...
  constructor(
    ...
    @Inject(PROVIDER_TOKENS.BASE_STREAM_HANDLER) private readonly baseStreamHandler: BaseStreamHandlerService
  ) {}
  ...
  async generate(@Body() generateRequest: GenerateRequest, @Res() res: Response) {
    ...
    const isCodeCompletionReq = this.prepareCodeCompletionRequest(generateRequest);
    ...
    if (isStream) {
      ...
      this.handleNonStreamingResponse(response, res, modelToUse);
    }
  }
  ...
  private isInvalidGenerateRequest(generateRequest: GenerateRequest, res: Response): boolean {
    if (!generateRequest || !generateRequest.prompt) {
      ...
      // 여기서 BadRequestException을 사용하도록 변경
      throw new BadRequestException('요청 본문이 비어 있거나 prompt 필드가 없습니다');
    }
  ...
  /**
   * 코드 자동완성 요청인 경우 적절한 시스템 프롬프트를 설정합니다.
   */
  private prepareCodeCompletionRequest(generateRequest: GenerateRequest): boolean {
    // 코드 자동완성 요청인지 확인
    ...
    return isCodeCompletionReq;
  }

  /**
   * 비스트리밍 응답을 처리합니다.
   */
  private handleNonStreamingResponse(response: any, res: Response, modelToUse: string): void {
    ...
    // SSE 헤더 설정 및 응답 전송
    this.baseStreamHandler.setupSSEHeaders(res);
